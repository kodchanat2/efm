...
<template>
  <Block v-bind="props" @update="emit('update', $event)" @duplicate="emit('duplicate')" @delete="emit('delete')">
    <div class="flex flex-col w-full max-w-sm">
      <div v-for="(option, index) in props.props?.options" :key="index" class="grid w-full grid-cols-[auto_1fr_0.5fr] gap-2 mt-2">
        <Icon
          icon="ic:round-radio-button-unchecked"
          class="text-xl text-muted-foreground self-center"
        />
        <Input
          type="text"
